// Embed the Tag and Send (User Experience) (Node.js)
// 
// To run this sample
//  1. Copy the file to your local machine and give .js extension (i.e. example.js)
//  2. Change "***" to appropriate values or setup your environment variables (export DOCUSIGN_INTEGRATOR_KEY="...")
...
var integratorKey = process.env.DOCUSIGN_INTEGRATOR_KEY || '***',	// Integrator Key associated with your DocuSign Integration
	...
	fileToSign = 'blank.pdf', // available in the "_sample_documents" folder
...
async.waterfall(
  [
	...
	/////////////////////////////////////////////////////////////////////////////////////
	// Step 2: Create Envelope (status="created")
	/////////////////////////////////////////////////////////////////////////////////////

	function createEnvelope(loginAccounts, next){
		...
		envelopesApi.createEnvelope(accountId, envDef, null, function (error, envelopeSummary, response) {
			...
			if (envelopeSummary) {
				...
				next(null, loginAccounts, envelopeSummary.envelopeId);
			}
		});
	},
	...
	function createSenderView(loginAccounts, envelopeId, next){
		var loginAccount = new docusign.LoginAccount();
		loginAccount = loginAccounts[0];
		var accountId = loginAccount.accountId;
